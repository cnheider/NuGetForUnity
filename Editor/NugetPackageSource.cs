namespace NuGetForUnity.Editor {
  ...
  public class NugetPackageSource {
    ...
    public string ExpandedPassword {
      get {
        return this.SavedPassword != null
                   ? Environment.ExpandEnvironmentVariables(name : this.SavedPassword)
                   : null;
      }
    }
    ...
    public List<NugetPackage> FindPackagesById(NugetPackageIdentifier package) {
      List<NugetPackage> found_packages;

      if (this.IsLocalPath) {
        if (!package.HasVersionRange) {
          var local_package_path = Path.Combine(path1 : this.ExpandedPath,
                                                path2 : string.Format("./{0}.{1}.nupkg",
                                                                      arg0 : package._Id,
                                                                      arg1 : package._Version));
          if (File.Exists(path : local_package_path)) {
            var local_package = NugetPackage.FromNupkgFile(nupkgFilepath : local_package_path);
            found_packages = new List<NugetPackage> {local_package};
          } else {
            found_packages = new List<NugetPackage>();
          }
        } else {
    ...
    public NugetPackage GetSpecificPackage(NugetPackageIdentifier package) {
      return this.FindPackagesById(package : package).FirstOrDefault();
    }
    ...
    public List<NugetPackage> Search(string search_term = "",
    ...
    /// <param name="number_to_skip">The number of packages to skip before fetching.</param>
    /// <returns>The list of available packages.</returns>
    List<NugetPackage> GetLocalPackages(string search_term = "",
                                        bool include_all_versions = false,
                                        bool include_prerelease = false,
                                        int number_to_get = 15,
                                        int number_to_skip = 0) {
      var local_packages = new List<NugetPackage>();
      ...
      if (Directory.Exists(path : path)) {
        var package_paths = Directory.GetFiles(path : path,
        ...
        foreach (var package_path in package_paths) {
          ...
          var existing_package = local_packages.FirstOrDefault(x => x._Id == package._Id);
    ...
    /// <returns></returns>
    List<NugetPackage> GetPackagesFromUrl(string url, string username, string password) {
      ...
      using (var response_stream =
          NugetHelper.RequestUrl(url : url,
                                 ...
                                 password : password,
                                 time_out : 5000)) {
        using (var stream_reader = new StreamReader(stream : response_stream)) {
          packages = NugetODataResponse.Parse(document : XDocument.Load(textReader : stream_reader));
    ...
    List<NugetPackage> GetLocalUpdates(IEnumerable<NugetPackage> installed_packages,
                                       bool include_prerelease = false,
                                       bool include_all_versions = false) {
    ...
    /// <returns>A list of all updates available.</returns>
    public List<NugetPackage> GetUpdates(IEnumerable<NugetPackage> installed_packages,
                                         bool include_prerelease = false,
                                         bool include_all_versions = false,
      ...
      for (var i = 0; i < installed_packages.Count(); i += 10) {
        var package_group = installed_packages.Skip(count : i).Take(10);
        ...
        try {
          ...
          var web_response = web_exception != null ? web_exception.Response as HttpWebResponse : null;
          if (web_response != null && web_response.StatusCode == HttpStatusCode.NotFound) {
 ...
 GetUpdatesFallback(installedPackages, includePrerelease, includeAllVersions, targetFrameworks, versionContraints);
    ...
    static void ComparePackageLists(List<NugetPackage> updates,
                                    List<NugetPackage> updates_replacement,
      ...
      foreach (var package in updates) {
        if (updates_replacement.Contains(item : package)) {
          ...
          matching_comparison.Append(value : package.ToString());
        } else {
          missing_comparison.Append(value : missing_comparison.Length == 0 ? "Missing: " : ", ");
          missing_comparison.Append(value : package.ToString());
        }
      }
      ...
      foreach (var package in updates_replacement) {
        if (!updates.Contains(item : package)) {
          ...
          extra_comparison.Append(value : package.ToString());
        }
      }
    ...
    /// <param name="version_contraints">The version constraints?</param>
    /// <returns>A list of all updates available.</returns>
    List<NugetPackage> GetUpdatesFallback(IEnumerable<NugetPackage> installed_packages,
                                          bool include_prerelease = false,
                                          bool include_all_versions = false,
                                          string target_frameworks = "",
                                          string version_contraints = "") {
      var stopwatch = new Stopwatch();
      stopwatch.Start();
      Debug.Assert(condition : string.IsNullOrEmpty(value : target_frameworks)
                               && string
                                   .IsNullOrEmpty(value : version_contraints)); // These features are not supported by this version of GetUpdates.
      ...
      foreach (var installed_package in installed_packages) {
        ...
        updates.AddRange(collection : package_updates.Skip(count : skip));
      }
